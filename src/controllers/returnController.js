...
exports.getReturnById = (req, res) => {
    ...
    db.query('SELECT * FROM pengembalian WHERE id_return = ?', [id_pengembalian], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
...
exports.createReturn = (req, res) => {
    ...
    const query = `INSERT INTO pengembalian (id_return, id_rental, tanggal_pengembalian, denda, kondisi_mobil) VALUES (?, ?, ?, ?, ?)`;
...
exports.updateReturn = (req, res) => {
    ...
    const query = `UPDATE pengembalian SET id_rental=?, tanggal_pengembalian=?, denda=?, kondisi_mobil=? WHERE id_return=?`;
...
exports.deleteReturn = (req, res) => {
    const { id_pengembalian } = req.params;
    db.query('DELETE FROM pengembalian WHERE id_return = ?', [id_pengembalian], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
